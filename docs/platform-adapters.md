...
宿主只提供具体 locator 和入口。例如 DSH adapter 把 authoring help 渲染为 `"$DSH_WORKSURFACE_CLI" help author`，把绑定 Session 的工作目录渲染为 `$DSH_SURFACE_DIR`。因此模型始终得到“这是什么”和“可以怎么用”，但平台路径、命令和生命周期不会进入平台无关设计。DSH persistent Bash 的真实 consumer 在每次命令收集当前 shellEnv overlay；动态 locator 仅用于诊断，不替代环境注入。
...
- `CodeFirstSurfacePort`：读取和应用 Revision、在共享文件事务中记录 Operation batch、解析宿主产生的外部输入、请求一次 Surface advance。
...
版本状态机由 `SurfaceContentRuntime` 实现，DSH port 直接委托。`FileWorkspace` 不依赖宿主和编辑器；可选 b2f adapter 只把消息文件事务接到同一草稿目录与锁，不自动发布。详细责任与验收见[核心与宿主边界](core-and-host-boundaries.md)。
