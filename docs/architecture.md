...
| `Design guidance` | `packages/design/src/model-guidance.ts` | 平台无关的模型选择规则、概念边界和绑定 Session 指令模板 | 无；由宿主 adapter 渲染并注入 |
...
| `DshCodeFirstSurfacePort` | `packages/dsh/src/code-first-surface-port.ts` | 将 DSH Session/Turn、tool completion 和候选 Revision 映射到 Runtime Surface port | 目标 Surface Event 与 DSH Turn |
