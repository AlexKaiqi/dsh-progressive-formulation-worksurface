...
> 本文只做代码导航，不承担主设计图职责。概念定义见[设计基线](design-baseline.md)和[核心与宿主边界](core-and-host-boundaries.md)，关系见[可交互系统设计图](interactive/worksurface-system.html)。

## 1. 组件与物理责任

| 组件 | 代码 | 当前责任 | 持久化结果 |
| --- | --- | --- | --- |
| `WorkSurfaceService` | `packages/dsh/src/service.ts` | DSH Cordis 装配、Host RPC、逐项隔离的作者目录扫描和 DSH adapter wiring | 间接写 Definition、Revision、Event streams |
| `Design guidance` | `packages/design/src/model-guidance.ts` | 平台无关的适用判断、概念边界和 Surface 工作指导模板 | 无；由宿主 adapter 渲染并注入 |
| `Progression Runtime` | `packages/runtime/src/{engine,code-first-orchestrator}.ts` | 平台无关的 replay、Activation、record/apply/settle、staged run、恢复和 advance 协议 | Registration、Input、Operation facts 与目标 Surface facts |
| `SurfaceSessionService` | `packages/dsh/src/session-surface.ts` | DSH 1:1 binding、Turn capability、兼容 emit、从当前版本来源恢复上下文 | `binding.json`、`context.json`、Surface events |
...
| `WorkSurfaceEngine` | `packages/runtime/src/engine.ts` | replay、Activation 派生、Operation record/execute/settle、reconcile；通过 Host port 投递 followup | Registration stream 与目标 Surface event / Host receipt |
...
| `FileWorkspace` | `packages/core/src/file-workspace.ts` | 通用草稿文件观察、CAS、共享写锁、可恢复投影 | workspace pending intents / receipts；不发布 Surface |
| `SurfaceContentRuntime` | `packages/runtime/src/surface-content-runtime.ts` | 精确 Contract 身份的 head 折叠、Surface 版本比较、发布、apply 恢复 | Surface revision Events 与 apply intents |
...
| `RuntimeEventStore` | `packages/core/src/runtime-event-store.ts` | Contract identity / producer / subject / payload 校验、subject-local seq 与幂等 append | `v5/events/` |
...
| `OperationLedgerStore` | `packages/core/src/orchestrate-ledger-store.ts` | record-before-effect、Surface 预留与 settle 恢复屏障、失败诊断 | `v5/operation-ledger/` |
...
| `CodeFirstOrchestrator` | `packages/runtime/src/code-first-orchestrator.ts` | Registration resolve、staged run、完整校验、恢复和 advance；通过 Surface port 访问宿主 | 上述 v5 私有事实与目标 Surface Event |
...
| `DshCodeFirstSurfacePort` | `packages/dsh/src/code-first-surface-port.ts` | 委托 SurfaceContentRuntime 处理版本，只映射 DSH Session/Turn、tool completion 和 advance | 目标 Surface Event 与 DSH Turn |
...
| v5 Surface head | `Revision` | `surface.revision.admitted/applied/published` Event | `SurfaceContentRuntime.recordedHead()` 按精确 Contract 身份 replay；没有可变 head 文件 |
...
| v5 Runtime input | `OrchestrateInputLedgerRecord` | WorkSurface EventRef 或 Host execution EventRef | recovery 先扫描目标 Event stream 补收，再续推未记录 input |
...
`ws publish` 使用当前 Turn capability 推导目标 Surface，委托 `SurfaceContentRuntime.publish` 提交文件版本；它独立于业务输出授权，`outputs: []` 的 Turn 也可以发布交付物。文件版本确认后，`ws emit` 使用当前 Turn capability binding，先 `syncAuthoringRegistrations()`，再按当前 Surface 的 `surfaceOutputFrom` route 解析唯一 Contract、校验 payload 并 append；wakeup 后 Runtime replay Input Ledger。Registration 在某个 Turn 内首次写入时不会追溯授权该 Turn：输出 Contract 只会进入下一 Turn 的 Brief。旧 `definition.json` 目录仍进入 v4 compatibility Engine。

`session/event` adapter 只把 DSH `tool/result` 变成带 execution subject、原始 seq 和确定性 id 的 `source=external` EventRef。执行输入在需要时回到 DSH Session Log 解析 `tool/call + tool/result`，只物化 turn、step、callId、toolName、status 与可选 errorCode；工具参数和结果正文不复制到 WorkSurface ledger。Runtime 不知道 DSH 的事件类型，只消费 Host port 提供的外部输入解析结果。
