...
> 状态：权威 UI 设计，基线日期 2026-08-31。本文只定义可删除投影的产品形态、视觉编码和交互；领域事实、DSH Session 集成与 Orchestrate 语义由[完整系统设计](worksurface-complete-design.md)定义。文档治理与实现入口见[文档索引](README.md)。
...
用户从 Surface identity、事件和 materialization adapters 重放出的列表显式选择拓扑锚点；conversation target 不充当 SurfaceId。Host 再根据 Orchestrate Instance bindings 与事件事实发现相关 Surface。锚点选择只保存在浏览器临时 UI 状态中，可随时丢弃；图允许循环，不按目录、时间戳、创建顺序或底层 persistence lineage 推断依赖。
...
点击 Surface 会把它设为新的显式拓扑锚点并重新发现相关子图；“进入推进”打开该锚点当前 adapter 唯一绑定的 DSH Session；点击条件或通路打开当前视图内的证据侧栏。锚点选择与 Session 导航是两个不同动作。Surface 的推进历史按 Episode 展示；当前 Session 只承载实际执行与 DSH 权威日志。Episode 可以引用多个 Turn / Step / Tool Call，UI 不得把其中任一层级自动当作 Episode。UI 可以按 Binding、Episode 边界与 EventRef 展示执行证据，但不能提供换绑、切换或给同一 Surface 新建第二个 Session 的入口。
...
| positive | `#238636` | Surface 上下文已发布新的 materialization snapshot | 对勾 |
| danger | `#cf222e` | Turn 失败、取消或 materialization publication conflict | 叉号 |

显式 `surface.completed` 使用盾牌对勾，与上下文发布的普通对勾区分。`turn/end`、`step/end` 和工具成功不能使用业务完成图标。
...
- Definition revision、Instance 与 subscription id；
- 角色绑定；
- 完整条件结构；
- 已收到和缺少的 selector 证据；
- 自定义 Event 的 payload 摘要，以及 inline value / content ref 的明确区分；
- activation 与 handler invocation；
- managed operation、目标 EventRef 与关联 Session/Turn；
- 失败及恢复信息。

引用 Surface 局部内容时，侧栏必须显示完整 `SurfaceId + adapter + locator + boundary`；adapter 可导航时可以打开对应局部。UI 不能只展示一段复制文本或易变标题而丢失地址，也不能把没有 boundary 的 working address 伪装成可重放证据。
...
标题优先级为：锁定 View 标题、普通 View 标题、可选 Surface Profile 或 materialization provider 派生标题、缩短 SurfaceId。文件型 provider 可以在 `surface.md` 存在时读取它，但通用回退不能要求该文件存在。
