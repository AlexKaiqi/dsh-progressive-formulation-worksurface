...
export interface CodeFirstSurfacePort {
  ...
  resolveDshInput(event: RuntimeEventRef): Promise<{ readonly surfaceId: string; readonly name: string; readonly payload: Readonly<Record<string, JsonValue>> }>
  apply(
    surfaceId: string,
...
export class CodeFirstOrchestrator {
  ...
  constructor(
    ...
    private readonly runner: SubprocessOrchestrateCodeRunner,
  ...
  /** Accept a future DSH Session fact without copying its payload out of the Session log. */
  acceptDsh(event: RuntimeEventRef, surfaceId: string, name = 'dsh.tool.completed'): Promise<void> {
    return this.serialize(async () => {
      if (event.source !== 'dsh' || event.subject.authority !== this.authority || event.subject.kind !== 'dsh-session') throw new WorkSurfaceError('invalid-working-copy', 'DSH adapter produced an invalid EventRef')
      for (const registrationId of await this.registrations.list()) {
        ...
        if (contract.name !== name || contract.scope.kind !== 'builtin') throw new WorkSurfaceError('canonical-corrupt', `DSH route '${name}' does not resolve its built-in Contract`)
        if (event.seq <= registration.historyBoundary[handle]!.dshEventSeq) continue
  ...
  private async run(registration: OrchestrateRegistrationRecord, triggerInputSeq: number): Promise<void> {
    ...
    for (const record of ledger.filter(record => record.inputSeq <= triggerInputSeq)) {
      const resolved = record.event.source === 'worksurface'
        ...
        : await this.surfaces.resolveDshInput(record.event)
