...
/** The engine addresses Surfaces; the Surface layer resolves each one's unique DSH Session. */
...
export class WorkSurfaceEngine {
  ...
  private async managedOperation(
    ...
    if (action.kind === 'followup') {
      ...
      const messageId = `worksurface:${sha256(`${registrationId}\0${activation.id}\0${action.operationKey}\0${targetSubject}`).slice(0, 40)}`
      ...
      const target = await this.followup(surfaceId, action.message, messageId)
      await this.appendRecord(registrationId, {
  ...
  private async recoverRecorded(registrationId: string, records: readonly OrchestrationRecord[]): Promise<void> {
    ...
    for (const [key, operation] of state.recordedOperations) {
      ...
      let target: EventRef | { readonly sessionId: string; readonly messageId: string }
      if (operation.targetSubject !== undefined && operation.event !== undefined) {
        ...
        const messageId = `worksurface:${sha256(`${registrationId}\0${operation.activationId}\0${operation.operationKey}\0${operation.targetSubject}`).slice(0, 40)}`
        target = await this.followup(operation.targetSubject.slice('surface:'.length), operation.message, messageId)
      } else {
  ...
  private followup(surfaceId: string, message: string, messageId: string): Promise<{ readonly sessionId: string; readonly messageId: string }> {
    ...
    return this.events.followupSurface(surfaceId, message, messageId)
  }
