...
每个 `canonical/surfaces/<surface-id>` 目录还包含 `session/header.json`、物化的 Session `HEAD.json` 和连续的不可变事件。Child Surface 在磁盘上仍与父级平级；父 Work Session 的 `child/created` 事件才是让 child 进入图的权威事实。`graphSnapshot()` 递归 fold 这些事件，不扫描当前 front matter 来猜测成员关系。详见 [`../../docs/work-session-storage.zh.md`](../../docs/work-session-storage.zh.md)。
