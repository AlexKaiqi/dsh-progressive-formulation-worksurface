...
`npm run eval:check` 会执行引用的 Web 行为测试，验证当前页面明确属于 Evidence 模式、无 iframe 的原生 `conversation.view`、连通拓扑重放、不同条件汇合点与可能/实际通路、关联 Session/Turn 与 publication、精确 Definition，以及已校验不可变 View YAML；浏览器不维护关系或执行状态，也不为尚未实现的 Design/Run 提供假入口。
