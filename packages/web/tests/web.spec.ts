...
describe('native WorkSurface topology view', () => {
  it('uses the DSH conversation view ring and delegates Surface progress to native Session navigation', async () => {
    ...
    expect(client).toContain("title: 'WorkSurface run evidence'")
    expect(client).toContain("title: 'WorkSurface 运行证据'")
    expect(client).toContain("subtitle: 'Evidence 模式：")
    ...
    expect(client).toContain('key: codeFirstGraphKey(snapshot, locale, subgraphId)')
    expect(client).toContain('function codeFirstGraphKey(snapshot, locale, activeSubgraphId)')
